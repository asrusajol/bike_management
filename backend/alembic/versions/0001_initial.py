...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("odometer_unit",  postgresql.ENUM("km", "miles", name="odometerunit", create_type=False), nullable=False, server_default="km"),
    ...
    op.create_table(
        ...
        sa.Column("updated_at",       sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.Column("bike_id",          postgresql.UUID(as_uuid=True), sa.ForeignKey("bikes.id", ondelete="CASCADE"), nullable=False),
        sa.Column("type",             postgresql.ENUM("service", "insurance", "tax", "custom", name="remindertype", create_type=False), nullable=False),
